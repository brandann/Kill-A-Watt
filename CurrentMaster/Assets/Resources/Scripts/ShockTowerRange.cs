...
public class ShockTowerRange : MonoBehaviour {
	...
	void Start () {
		...
		shockTowerShoot = (GameObject)(Instantiate(shockRangePrefab, this.transform.parent.position, Quaternion.Euler(0, 0, 0)));
	}
	
	// Update is called once per frame
	void Update () {
	shockTowerShoot.GetComponent<ShockTowerShoot>().parentsOwner = this.transform.parent.gameObject.GetComponent<Tower>().myOwner;
